...
df_wc_ranked = df.merge(rank[["country_full", "total_points", "previous_points", "rank", "rank_change", "rank_date"]],
                        left_on=["date", "home_team"], right_on=["rank_date", "country_full"]).drop(
...
home_team.columns = [h.replace("home_", "").replace("_home", "").replace("away_", "suf_").replace("_away", "_suf") for h
                     in home_team.columns]

away_team.columns = [a.replace("away_", "").replace("_away", "").replace("home_", "suf_").replace("_home", "_suf") for a
                     in away_team.columns]
...
for idx, teams in enumerate(matches):
    ...
    if draw == False:
        print("小组 %s - %s vs. %s: %s 获胜，胜率： %.2f" % (
            teams[0], country_name[teams[1]] + "(" + teams[1] + ")", country_name[teams[2]] + "(" + teams[2] + ")",
            country_name[winner] + "(" + winner + ")", winner_proba))
    ...
    if p == "16 强":
        ...
        for i in range(0, len(playoffs[p]), 1):
            ...
            if team_1_prob < team_2_prob:
                print("%s vs. %s: %s 晋级，概率： %.2f" % (
                    country_name[home] + "(" + home + ")", country_name[away] + "(" + away + ")",
                    country_name[away] + "(" + away + ")", team_2_prob))
                ...
                print("%s vs. %s: %s 晋级，概率： %.2f" % (
                    country_name[home] + "(" + home + ")", country_name[away] + "(" + away + ")",
                    country_name[home] + "(" + home + ")", team_1_prob))
